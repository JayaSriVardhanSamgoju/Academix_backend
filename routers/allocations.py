...
def auto_allocate(
    ...
    for es in all_exam_students_rels:
        ...
        # --- Detention Logic (Edge Case 1.4) ---
        # Filter out detained students
        if getattr(stu, "academic_status", "") == "DETAINED":
            detained_students.append({
                "student_id": stu.roll_number,
                "reason": "Academic Detention"
            })
            continue
    ...
    return {
        ...
        "detention_exclusion_report": {
            "total_excluded": len(detained_students),
            "excluded_students": detained_students
        }
    }
...
def assign_invigilation(
    req: InvigilationRequest,
    current_user: Annotated[models.User, Depends(auth_router.get_current_active_user)],
    db: Session = Depends(get_db)
):
    """Assign invigilation duty to a faculty member (Teaching or Non-Teaching)."""
    # Check role: Seating Manager or Admin
    # user.role is a string
    if current_user.role not in ["Seating Manager", "Admin"]:
    ...
    # Check if already assigned for this exam?
    existing = db.query(models.InvigilationDuty).filter(
        models.InvigilationDuty.faculty_id == req.faculty_id,
        models.InvigilationDuty.exam_id == req.exam_id
    ).first()
    
    if existing:
        return {"message": "Faculty already assigned to this exam"}
    
    duty = models.InvigilationDuty(
    ...
    db.add(duty)
    db.commit()
    return {"message": "Invigilation assigned successfully"}

@router.get("/invigilation/{exam_id}")
def get_exam_invigilations(
    exam_id: int, 
    db: Session = Depends(get_db)
):
    """
    Get invigilation status for all rooms used in an exam.
    Returns list of { room_id, room_name, assigned_faculty_id, assigned_faculty_name, status }
    """
    # 1. Find all rooms used in this exam
    # Distinct room_ids from SeatAllocation
    used_room_ids = db.query(models.SeatAllocation.room_id).filter(
        models.SeatAllocation.exam_id == exam_id
    ).distinct().all()
    ...
    rooms = db.query(models.Room).filter(models.Room.id.in_(used_room_ids)).all()
    
    # 2. Find existing assignments
    assignments = db.query(models.InvigilationDuty).filter(
        models.InvigilationDuty.exam_id == exam_id
    ).options(joinedload(models.InvigilationDuty.faculty).joinedload(models.Faculty.user)).all()
    
    duty_map = { a.room_id: a for a in assignments }
    
    results = []
    for r in rooms:
        duty = duty_map.get(r.id)
        results.append({
            "room_id": r.id,
            "room_name": r.name,
            "capacity": r.capacity,
            "assigned_faculty_id": duty.faculty_id if duty else None,
            "assigned_faculty_name": duty.faculty.user.name if duty and duty.faculty and duty.faculty.user else None,
            "assigned_faculty_dept": duty.faculty.department if duty and duty.faculty else None,
            "status": "Assigned" if duty else "Pending"
        })
