...
def enroll_student(course_id: int, payload: schemas.CourseEnrollmentCreate, db_session: Session = Depends(get_db)):
    course = db_session.query(models.Course).filter(models.Course.id == course_id).first()
    if not course:
        raise HTTPException(status_code=404, detail="Course not found")
...
def get_student_enrollments(
    ...
    try:
        ...
        courses = db_session.query(models.Course).filter(models.Course.id.in_(course_ids)).all()
